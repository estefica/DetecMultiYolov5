...
def labels_multi():
    ...
    divide_images.escalado_inicial(path_ima, path_ima_t, path_s_files, path_s_filest)
    for grid in gridl:#range(2,10)
        #print('entre al grid')
        for f in os.listdir(path_imagenes):
            ...
            if f.endswith('.jpg'):
                ...
                if grid==1:
                    ...
                    path_save_filest = f'/content/labels/{fn}_{grid}'
                else:
                    path_save_files = f'/content/random/images/{fn}_{grid}'
                    path_save_filest = f'/content/random/labels/{fn}_{grid}'
                alto_deseado = 720
                ancho_deseado = 960
                ...
                if os.path.exists(path_imagenes+fn+'.txt'):
                        try:
                            ...
                            if labels.shape[1] == 5:
                                ...
                                resolucion_original_alto = int(resolucion_original_ancho*((img.shape[0])/(img.shape[1])))#no mayor a 10
                                resolucion_original_alto =int(720*(int(round( resolucion_original_alto/720))))
                                imgResize = cv2.resize(img, (resolucion_original_ancho, resolucion_original_alto))  # width and height
                                #print(img.shape)
                                ...
                                ###DEL .txt original se tiene el centro, y ancho y alto
                                #print(f'limites:{limites_ancho},limites h:{limites_alto}')
                                for boxes in range(labels.shape[0]):
                                    # Ubicar y cambiar de normalizados a nuevas escalas
                                    linen = 0
                                    ...
                                    #print(f'puntos x: {puntos_x}')
                                    #print(f'puntos y: {puntos_y}')
                                    comprobar = 0
                                    #print(f'htotal:{h_total}')
                                    #print(f'wtotal:{w_total}')
                                    #cv2.rectangle(imgResize,(p1x,p1y),(p4x,p4y),(0,255,0),5)
                                ...
                                if comprobar == 0:
                                    ...
                                    for x in range(len(limites_ancho) - 1):
                                        ...
                                        for y in range(len(limites_alto) - 1):
                                            ...
                                            c = [[(0, 0), (0, 0), (0, 0), (0, 0)]]  # CONTIENEN LOS PUNTOS OBTENIDOS DESDE EL .TXT
                                            ...
                                            for rec_x in puntos_x:  # primero los puntos en X
                                                cont += 1
                                                aux = cont - 1
                                                if rec_x in range(limites_ancho[x], limites_ancho[x + 1] + 1):  # Punto entre el EJE X, DE EXTREMO A EXTREMO
                                                    if puntos_y[aux] in range(limites_alto[y],
                                                        ...
                                                        puntos_bounding(c) #OBTENGO IMAGENES Y LABELS DE LOS PUNTOS EXTREMOS P1,P2,P3,P4 y objeto region guarda a cada imagen
                                                ...
                                                if cont_puntos == 3:
                                                    ...
                                                    if cuadros_altof > 0:
                                                        ...
                                                        if reset == 0:
                                                            for iz in range(len(limites_alto_complemento) - 1):
                                                                imgcrop = imgResize[
                                                                          limites_alto_complemento[iz]:limites_alto_complemento[iz + 1],
                                                                          borde_iz1: borde_iz2]
                                                                crear_doc_texto(c1p1x, limites_alto_complemento[iz], borde_iz2,
                                                                                 limites_alto_complemento[iz + 1], path_save_filest +'obj{}_iz{}.txt',
                                                                                 [objeto_region[key][6]], imagen_numero, borde_iz1,
                                                                                 limites_alto_complemento[iz],final)

                                                                if wabs >=0.38*w_total[indicep] and habs >=0.38*h_total[indicep]:
                                                                    cv2.imwrite(path_save_files+'obj{}_iz{}.jpg'.format([objeto_region[key][6]], imagen_numero),imgcrop)
                                                                    imagen_numero += 1 #wabs >=0.38*w_total[indicep] and habs >=0.5*h_total[indicep]
                                                        ...
                                                        for der in range(len(limites_alto_complemento) - 1):
                                                           ...
                                                           #                15)
                                                           imgcrop = imgResize[
                                                                     limites_alto_complemento[der]:limites_alto_complemento[
                                                                         der + 1], borde_der1: borde_der2]
                                                           if reset == 0:
                                                               crear_doc_texto(borde_der1, limites_alto_complemento[der], c2p2x,
                                                                               limites_alto_complemento[der + 1], path_save_filest+'obj{}_der{}.txt',
                                                                               [objeto_region[key][6]], imagen_numero, borde_der1,
                                                           ...
                                                           if wabs >=0.38*w_total[indicep] and habs >=0.38*h_total[indicep]:
                                                               cv2.imwrite(
                                                                   path_save_files+'obj{}_der{}.jpg'.format([objeto_region[key][6]], imagen_numero),
                                                                   imgcrop)
                                                               imagen_numero += 1
                                                    ...
                                                    if cuadros_anchof > 0:
                                                        ...
                                                        for sup in range(len(limites_ancho_complemento) - 1):
                                                            ...
                                                            #             (0, 255, 0), 15)
                                                            imgcrop = imgResize[borde_sup1: borde_sup2,limites_ancho_complemento[sup]:limites_ancho_complemento[sup + 1]]
                                                            crear_doc_texto(limites_ancho_complemento[sup], c1p1y,
                                                                            limites_ancho_complemento[sup + 1], borde_sup2,
                                                                            path_save_filest+'obj{}_sup{}.txt', [objeto_region[key][6]],
                                                                            imagen_numero, limites_ancho_complemento[sup],
                                                                            borde_sup1,final)
                                                            if wabs >=0.38*w_total[indicep] and habs >=0.38*h_total[indicep]:
                                                                cv2.imwrite(path_save_files+'obj{}_sup{}.jpg'.format([objeto_region[key][6]], imagen_numero),                                                        imgcrop)
                                                                imagen_numero += 1
                                                        ...
                                                        for inf in range(len(limites_ancho_complemento) - 1):
                                                            ...
                                                            imgcrop = imgResize[borde_inf1: borde_inf2,
                                                                      limites_ancho_complemento[inf]:limites_ancho_complemento[
                                                                          inf + 1]]

                                                            crear_doc_texto(limites_ancho_complemento[sup], borde_inf1,
                                                                            limites_ancho_complemento[sup + 1], c3p3y,
                                                                            path_save_filest+'obj{}_inf{}.txt', [objeto_region[key][6]],
                                                                            imagen_numero, limites_ancho_complemento[inf],
                                                                            borde_inf1,final)
                                                            if wabs >=0.38*w_total[indicep] and habs >=0.38*h_total[indicep]:
                                                                cv2.imwrite(path_save_files+ 'obj{}_inf{}.jpg'.format([objeto_region[key][6]], imagen_numero),imgcrop)
                                                                imagen_numero += 1
                                                    ...
                                                    if cuadros_anchof * cuadros_altof != 0:
                                                        for dx in range(len(limites_ancho_complemento) - 1):
                                                            for dy in range(len(limites_alto_complemento) - 1):
                                                                ...
                                                                imgcrop = imgResize[
                                                                          limites_alto_complemento[dy]:limites_alto_complemento[
                                                                              dy + 1],
                                                                          limites_ancho_complemento[dx]: limites_ancho_complemento[
                                                                              dx + 1]]
                                                                ...
                                                                crear_doc_texto(limites_ancho_complemento[dx],
                                                                                ...
                                                                                path_save_filest+'obj{}_centro{}.txt', [objeto_region[key][6]],
                                                                                imagen_numero, limites_ancho_complemento[dx],
                                                                                limites_alto_complemento[dy],final)
                                                                if wabs >=0.38*w_total[indicep] and habs >=0.38*h_total[indicep]:
                                                                    cv2.imwrite(path_save_files + 'obj{}_centro{}.jpg'.format(
